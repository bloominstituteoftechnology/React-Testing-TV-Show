...
test('The Episodes component renders', () => {
  render(<Episodes episodes={episodes} />)
})

test('The Episode displays duration', () => {
  render(<Episodes episodes={episodes} />)

  screen.getAllByText(/minutes/i).map(minutesSelection => {
    expect(minutesSelection.innerHTML).not.toEqual('minutes')
  })
})