...
test('Episodes renders a complete episode list by season',()=>{
    const {getAllByText,rerender,queryAllByText}=render(<Episodes episodes={[]}/>);
    const initialEpisodes=queryAllByText(/episodes/i);
    expect(initialEpisodes).toHaveLength(0);
    rerender(<Episodes episodes={episodes}/>);
    const updatedEpisodes=getAllByText(/episode/i);
    expect(updatedEpisodes).toHaveLength(8)
})
