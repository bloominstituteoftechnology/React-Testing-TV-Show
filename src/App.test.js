...
//dummy data of 4 episodes from season one.
const DummyData = 
{
    "id": 2993,
...
test('render season data with dropdown clicked', async () => {
    mockFetchShow.mockResolvedValueOnce({
        data:DummyData
    })
    render(<App/>);
    ...
    const firstSeasonTab= await screen.findByText(/season 1/i);
    expect(firstSeasonTab).toBeInTheDocument()
    userEvent.click(firstSeasonTab)
})




//this works as well to render the episodes with the API

// jest.mock('./api/fetchShow')

// test('renders tv show with calling the API', async () => {
//     mockFetchShow()
