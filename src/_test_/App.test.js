...
import {
	...
	formatSeasons as mockFormatSeasons,
} from '../utils/formatSeasons';
...
test('Render App component', async () => {
	...
	const { getByTestId, queryAllByTestId, rerender } = render(<App />);
	await waitFor(() => {
		const select = getByTestId('select');
		expect(select).toBeTruthy();

		userEvent.selectOptions(select, ['Season 1']);
		expect(getByTestId('season1').selected).toBe(true);
