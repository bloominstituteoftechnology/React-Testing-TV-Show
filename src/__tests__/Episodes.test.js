...
const showData=[
 ...
 test('test rerender correctly', async () => {
    
    const { getByText, queryByTestId, rerender,getAllByText } = render(<Episodes episodes={[]}/>);

    expect(queryByTestId('tId')).not.toHaveTextContent('Jim about a call she')

    
    rerender(<Episodes episodes={showData} />)
    expect(queryByTestId('tId')).toHaveTextContent('Jim about a call she')
    expect(getAllByText(/chapter/i)).toHaveLength(26);
