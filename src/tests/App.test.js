...
import { render, screen, queryByText, getByText,fireEvent } from '@testing-library/react'
import App from '../App'
...
test('renders this shiz', async () => {
    mockFetchShow.mockResolvedValueOnce(episodesData)
    render(<App/>)

    const dog = screen.findByText('Select a season')
    selectEvent.select(dog)
